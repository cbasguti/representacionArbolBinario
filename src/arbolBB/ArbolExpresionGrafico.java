...
public class ArbolExpresionGrafico extends JPanel 
{
    ...
    private Dimension calcularTamañoSubarbol(NodoBinario n) 
    {
          ...
          Dimension ld = calcularTamañoSubarbol(n.retornaLigaIzq());
          Dimension rd = calcularTamañoSubarbol(n.retornaLigaDer());
    ...
    private void calcularPosicion(NodoBinario n, int left, int right, int top) 
    {
      ...
      Dimension ld = (Dimension) subtreeSizes.get(n.retornaLigaIzq());
      if (ld == null) 
          ld = empty;
      
      Dimension rd = (Dimension) subtreeSizes.get(n.retornaLigaDer());
      if (rd == null) 
          rd = empty;
      ...
      int width = fm.stringWidth(n.retornaDato()+"");
 
      posicionNodos.put(n,new Rectangle(center - width/2 - 3, top, width + 6, fm.getHeight()));
      
      calcularPosicion(n.retornaLigaIzq(), Integer.MAX_VALUE, center - child2child/2, top + fm.getHeight() + parent2child);
      calcularPosicion(n.retornaLigaDer(), center + child2child/2, Integer.MAX_VALUE, top + fm.getHeight() + parent2child);
    }
    ...
    private void dibujarArbol(Graphics2D g, NodoBinario n, int puntox, int puntoy, int yoffs) 
    {
     ...
     g.draw(r);
     g.drawString(n.retornaDato()+"", r.x + 3, r.y + yoffs);
   
     if (puntox != Integer.MAX_VALUE)
       
     g.drawLine(puntox, puntoy, (int)(r.x + r.width/2), r.y);
     
     dibujarArbol(g, n.retornaLigaIzq(), (int)(r.x + r.width/2), r.y + r.height, yoffs);
     dibujarArbol(g, n.retornaLigaDer(), (int)(r.x + r.width/2), r.y + r.height, yoffs);
